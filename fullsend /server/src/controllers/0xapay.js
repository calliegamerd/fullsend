...
async function createDepositAddress() {
  try { 
    return {
      ...
      trx: await makeReq("TRX"),
      bnb: await makeReq("BNB"),
      dgb: await makeReq("DGB"),
      sol: await makeReq("SOL"),
      ton: await makeReq("TON"),
      matic: await makeReq("MATIC"),
      bch: await makeReq("BCH"),
      shib: await makeReq("SHIB"),
    }
  } catch (error) {
    console.log(error);
  }
